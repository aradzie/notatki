...
def test_markdown_to_html_without_image_resolver_leaves_src_unchanged() -> None:
  assert markdown_to_html("![alt](diagram.png)") == ('<p><img src="diagram.png" alt="alt" /></p>\n')
...
def test_markdown_to_html_with_math() -> None:
  assert markdown_to_html("a \\(inline\\) and \\[display\\] b") == (
    '<p>a <span class="math">\\(inline\\)</span> and <span class="display-math">\\[display\\]</span> b</p>\n'
  )
  assert markdown_to_html("a $inline$ and $$display$$ b") == (
    '<p>a <span class="math">\\(inline\\)</span> and <span class="display-math">\\[display\\]</span> b</p>\n'
  )
...
def test_markdown_to_html_preserves_raw_html_tags_but_escapes_stray_chars() -> None:
  ...
  assert markdown_to_html("<b>bold</b> and <sub>sub</sub>") == ("<p><b>bold</b> and <sub>sub</sub></p>\n")
  # Stray "<", ">", "&" that aren't part of a recognized HTML tag are still escaped.
  assert markdown_to_html("1 < 2 and 3 > 4 and a & b") == ("<p>1 &lt; 2 and 3 &gt; 4 and a &amp; b</p>\n")


def test_markdown_to_html_escapes_html_special_chars_in_math() -> None:
  assert markdown_to_html("\\(a & b > c\\)") == ('<p><span class="math">\\(a &amp; b &gt; c\\)</span></p>\n')
  assert markdown_to_html("\\[a & b > c\\]") == ('<p><span class="display-math">\\[a &amp; b &gt; c\\]</span></p>')
